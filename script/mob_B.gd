...
func _ready():
	%BunnyB.play_walk()  # Play walk animation
...
func take_damage():
	...
	health -= 1
	%BunnyB.play_hurt()  # Play hurt animation
...
func _destroy_mob():
	emit_signal("mob_destroyed",self, 250)  # Emit the signal that the mob is destroyed and give score 250
