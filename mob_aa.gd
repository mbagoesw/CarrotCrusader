...
func _ready():
	%bunny_aaa.play_walk()  # Play walk animation
...
func take_damage():
	...
	health -= 1
	%bunny_aaa.play_hurt()  # Play hurt animation
...
func _destroy_mob():
	emit_signal("mob_destroyed",self, 100)  # Emit the signal that the mob is destroyed and give score 100
